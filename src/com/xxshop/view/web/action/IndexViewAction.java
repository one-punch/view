...
import com.xxshop.view.web.tools.GoodsFloorViewTools;
import com.xxshop.view.web.tools.GoodsViewTools;
import com.xxshop.view.web.tools.NavViewTools;
...
import org.springframework.web.servlet.ModelAndView;
 ...
 public class IndexViewAction
 {
   ...
   public ModelAndView top(HttpServletRequest request, HttpServletResponse response)
   {
     ...
     for (Object obj = user_cart.iterator(); ((Iterator)obj).hasNext(); ) { 
				StoreCart sc = (StoreCart)((Iterator)obj).next();
       boolean sc_add = true;
     ...
     boolean sc_add;
     for (Iterator iter = cookie_cart.iterator(); ((Iterator)iter).hasNext(); ) { 
				StoreCart sc = (StoreCart)((Iterator)iter).next();
       sc_add = true;
       for (StoreCart sc1 : cart) {
         if (sc1.getStore().getId().equals(sc.getStore().getId())) {
           sc_add = false;
     ...
     if (cart != null) {
       for (Iterator iter2 = cart.iterator(); ((Iterator)iter2).hasNext(); ) { 
					StoreCart sc = (StoreCart)((Iterator)iter2).next();
         if (sc != null) {
   ...
   public ModelAndView error404(HttpServletRequest request, HttpServletResponse response) {
     ModelAndView mv = new JModelAndView("404.html", 
       this.configService.getSysConfig(), 
       this.userConfigService.getUserConfig(), 1, request, response);
     String view_type = CommUtil.null2String(request.getSession(
       false).getAttribute("xxshop_view_type"));
