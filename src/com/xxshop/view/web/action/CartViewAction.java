...
public class CartViewAction {
	...
	private List<StoreCart> cart_calc(HttpServletRequest request) {
				...
				if (user.getStore() != null) {
					...
					List store_cookie_cart = this.storeCartService
							.query("select obj from StoreCart obj where (obj.cart_session_id=:cart_session_id or obj.user.id=:user_id) and obj.sc_status=:sc_status and obj.store.id=:store_id",
									params, -1, -1);
					for (Iterator iter11 = store_cookie_cart.iterator(); iter11
							.hasNext();) {
						StoreCart scTmp = (StoreCart) iter11.next();
						for (GoodsCart gc : ((StoreCart) scTmp).getGcs()) {
							gc.getGsps().clear();
							this.goodsCartService.delete(gc.getId());
						}
						this.storeCartService.delete(scTmp.getId());
					}
				}
		...
		for (Iterator iter11 = user_cart.iterator(); iter11.hasNext();) {
			StoreCart sc = (StoreCart) iter11.next();
			boolean sc_add = true;
		...
		for (Iterator iter12 = cookie_cart.iterator(); iter12.hasNext();) {
			StoreCart sc = (StoreCart) iter12.next();
			boolean sc_add = true;
			for (StoreCart sc1 : cart) {
				if (sc1.getStore().getId().equals(sc.getStore().getId())) {
					sc_add = false;
    ...
    public void add_goods_cart(HttpServletRequest request, HttpServletResponse response, String id, String count, String price, String gsp, String buy_type)
    {
      ...
      for (StoreCart usc : user_cart) {
        boolean sc_add = true;
        for (StoreCart sc1 : cart) {
          if (sc1.getStore().getId().equals(usc.getStore().getId())) {
            sc_add = false;
          }
        }
        if (sc_add) {
      ...
      for (StoreCart csc : cookie_cart) {
        boolean sc_add = true;
        for (StoreCart sc1 : cart) {
          if (sc1.getStore().getId().equals(csc.getStore().getId())) {
            sc_add = false;
            for (GoodsCart gc : csc.getGcs()) {
              gc.setSc(sc1);
              this.goodsCartService.update(gc);
            }
            this.storeCartService.delete(csc.getId());
          }
        }
        if (sc_add) {
          cart.add(csc);
      ...
      for (StoreCart storeCart : cart)
        for (GoodsCart gc : storeCart.getGcs())
      ...
      if (add) {
        ...
        nsc.setStore(goods.getGoods_store());
        if (((String)type).equals("save")) {
         nsc.setAddTime(new Date());
          this.storeCartService.save(nsc);
        } else {
          this.storeCartService.update(nsc);
        }
        ...
        ((GoodsCart)obj).setSc(nsc);
        ((GoodsCart)obj).setSpec_info(spec_info);
        this.goodsCartService.save((GoodsCart)obj);
        nsc.getGcs().add((GoodsCart)obj);
        double cart_total_price = 0.0D;
        for (Iterator goodsCarts = nsc.getGcs().iterator(); ((Iterator)goodsCarts).hasNext(); ) { 
	 					GoodsCart gc1 = (GoodsCart)((Iterator)goodsCarts).next();
        ...
        nsc.setTotal_price(BigDecimal.valueOf(
          CommUtil.formatMoney(Double.valueOf(cart_total_price))));
        if (user == null)
          nsc.setCart_session_id(cart_session_id);
        else {
          nsc.setUser(user);
        }
        if (((String)type).equals("save")) {
        ...
        for (StoreCart sc1 : cart) {
          if (sc1.getStore().getId().equals(nsc.getStore().getId())) {
            cart_add = false;
          }
        }
        if (cart_add) {
      ...
      for (Object storeCart = cart.iterator(); ((Iterator)storeCart).hasNext(); ) { StoreCart sc1 = (StoreCart)((Iterator)storeCart).next();
	...
	public void goods_count_adjust(HttpServletRequest request,
		...
		if (cart_type.equals("")) {
			if (goods.getGroup_buy() == 2) {
				...
				if (gg.getGg_count() >= CommUtil.null2Int(count))
					for (Iterator iter4 = cart.iterator(); iter4.hasNext();) {
						StoreCart sc4 = (StoreCart) iter4.next();
						for (int i = 0; i < ((StoreCart) sc4).getGcs().size(); i++) {
							GoodsCart gc4 = (GoodsCart) ((StoreCart) sc4)
									.getGcs().get(i);
							GoodsCart gc1 = gc4;
							if (gc4.getId().toString().equals(cart_id)) {
								((StoreCart) sc4)
										.setTotal_price(BigDecimal.valueOf(CommUtil.add(
												...
												Double.valueOf((CommUtil
														...
														* CommUtil
																.null2Double(gc4
																		.getPrice())))));
								gc4.setCount(CommUtil.null2Int(count));
								gc1 = gc4;
								((StoreCart) sc4).getGcs().remove(gc4);
								((StoreCart) sc4).getGcs().add(gc1);
								goods_total_price = CommUtil.null2Double(gc1
										.getPrice()) * gc1.getCount();
								this.storeCartService.update((StoreCart) sc4);
							}
						}
					}
				else {
				...
				for (Iterator iter3 = cart.iterator(); ((Iterator) iter3)
						.hasNext();) {
					StoreCart sc3 = (StoreCart) ((Iterator) iter3).next();
					for (int i = 0; i < sc3.getGcs().size(); i++) {
						GoodsCart gc3 = (GoodsCart) sc3.getGcs().get(i);
						GoodsCart gc1 = gc3;
						if (gc3.getId().toString().equals(cart_id)) {
							sc3.setTotal_price(BigDecimal.valueOf(CommUtil.add(
									sc3.getTotal_price(),
									Double.valueOf((CommUtil.null2Int(count) - gc3
											.getCount())
											* Double.parseDouble(gc3.getPrice()
													.toString())))));
							gc3.setCount(CommUtil.null2Int(count));
							gc1 = gc3;
							sc3.getGcs().remove(gc3);
							sc3.getGcs().add(gc1);
		...
		if (cart_type.equals("combin")) {
			if (goods.getGoods_inventory() >= CommUtil.null2Int(count))
				for (Iterator iter2 = cart.iterator(); ((Iterator) iter2)
						.hasNext();) {
					StoreCart sc2 = (StoreCart) ((Iterator) iter2).next();
					for (int i = 0; i < sc2.getGcs().size(); i++) {
						gc = (GoodsCart) sc2.getGcs().get(i);
						GoodsCart gc1 = (GoodsCart) gc;
						if (((GoodsCart) gc).getId().toString().equals(cart_id)) {
							sc2.setTotal_price(BigDecimal.valueOf(CommUtil.add(
									sc2.getTotal_price(),
		...
		for (Iterator iter1 = cart.iterator(); ((Iterator) iter1).hasNext();) {
			StoreCart sc1 = (StoreCart) ((Iterator) iter1).next();
			if (sc1.getStore().getId().equals(CommUtil.null2Long(store_id))) {
				((Map) map).put("sc_total_price", Float.valueOf(CommUtil
						.null2Float(sc1.getTotal_price())));
			}
		}
	...
	@SecurityMapping(title = "查看购物车", value = "/goods_cart1.htm*", rtype = "buyer", rname = "购物流程1", rcode = "goods_cart", rgroup = "在线购物", display = false, rsequence = 0)
	...
	public ModelAndView goods_cart3(HttpServletRequest request,
		...
		if (cart != null) {
			if (CommUtil.null2String(cart_session1).equals(cart_session)) {
				...
				Cookie[] cookies = request.getCookies();
				if (cookies != null) {
					Cookie[] arrayOfCookie1 = cookies;
					// GoodsCart localGoodsCart1 = (arrayOfCookie1 =
					// cookies).length;
					// for (gc = 0; gc < localGoodsCart1; gc++) { Cookie cookie
					// = arrayOfCookie1[gc];
					for (int i = 0; i < arrayOfCookie1.length; i++) {
